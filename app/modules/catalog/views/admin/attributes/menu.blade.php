...
if (isset($element) && is_object($element)) {
    ...
    if (Allow::action($module['group'], 'attributes_create')) {
        ...
        $array2 = $array;
        if (isset($element) && is_object($element)
            && isset($element->attributes_group)
            && is_object($element->attributes_group)
        )
            $array2['group'] = $element->attributes_group->id;

        $menus[] = array(
                'link' => URL::route('catalog.attributes_groups.create', $array2), 'title' => 'Добавить группу', 'class' => 'btn btn-primary'
        );
    }
}
